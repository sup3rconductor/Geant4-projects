...
G4VPhysicalVolume* DetDetectorConstruction::Construct()
{
	...
	/* G4double h_xpos = X0;
	G4double h_ypos = Y0 - 0.5 * ScrWidth + StartPosOtv;
	G4double h_zpos = 0.5 * ScrHeight; */
	G4double H_x = 0 * mm;
	...
	G4SubtractionSolid* solidScintplate[NHoles] = {NULL};
	...
	{
		solidScintplate[hole] = new G4SubtractionSolid("scintplate_s", solidScr, solidHole, HoleRot, G4ThreeVector(H_x, H_y, H_z));
		H_y += distance;
	}

	logicScintplate = new G4LogicalVolume(solidScintplate[hole], Scint, "scintplate_l");
	physScintplate[0][0][0] = new G4PVPlacement(0, G4ThreeVector(0., 0., 0.), logicScintplate, "scintplate", logicHollow, false, 0, checkOverlaps);
